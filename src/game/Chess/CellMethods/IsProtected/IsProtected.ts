...
export class IsProtected {
	static Diagonal(somePiece: Piece, target: Cell): boolean {
		...
		for (let i = 1; i < absY; i++) {
			if (
				IsEmpty.Cell(
					somePiece.cell.board.getCells(
						somePiece.cell.y + dy * i,
						somePiece.cell.x + dx * i
					)
				) ||
				(!IsEmpty.Cell(
					somePiece.cell.board.getCells(
	...
	static Vertical(somePiece: Piece, target: Cell): boolean {
		...
		for (let x = min + 1; x < max; x++) {
			if (
				...
				(!IsEmpty.Cell(somePiece.cell.board.getCells(somePiece.cell.y, x)) &&
					somePiece.cell.board.getCells(somePiece.cell.y, x).piece?.name ===
						PieceNames.KING &&
					somePiece.cell.board.getCells(somePiece.cell.y, x).piece?.color !==
						somePiece.color)
			) {
				count += 1;
			}
	...
	static Horizontal(somePiece: Piece, target: Cell): boolean {
		...
		for (let y = min + 1; y < max; y++) {
			if (
				...
				(!IsEmpty.Cell(somePiece.cell.board.getCells(y, somePiece.cell.x)) &&
					somePiece.cell.board.getCells(y, somePiece.cell.x).piece?.name ===
						PieceNames.KING &&
					somePiece.cell.board.getCells(y, somePiece.cell.x).piece?.color !==
						somePiece.color)
			) {
				count += 1;
			}
